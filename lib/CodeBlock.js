...
import { d as ne, A as sa, Y as oa, a2 as la } from "./RichTextEditor-C0nuBmeS.js";
    ...
    if (t[d]) for (var u = 0, f = t[d], E = Array.isArray(f) ? f : [f]; u < E.length; ++u) {
      ...
      for (var w = E[u], g = w.pattern || w, p = St(w.inside), v = w.lookbehind, h = w.greedy && g.global, _ = w.alias, a = n, i = r; a && (!c || i < c[2]); i += a[0].length, a = a[1]) {
        ...
        if (!(l instanceof be)) {
          ...
          if (s && s[0]) {
            for (var x = s.index, T = s[0], k = l.slice(x + T.length), O = i + l.length, P = new be(d, p ? ke(T, p) : T, T, _), C = a, Z = 0, H; C = C[1], Z++ < o; ) ;
...
$(re, "class-name", {
  ...
  "type-list": {
    ...
    pattern: N(
      ...
      [Vn, qe, J, ge, Ve.source, Ue, "\\bnew\\s*\\(\\s*\\)"]
    ),
    lookbehind: !0,
    inside: {
      "record-arguments": {
...
const Ua = (e, t, ...n) => {
  ...
  const a = Ga(), i = a.firstChild, l = i.children, o = l[0], s = o.firstChild, b = { language: "text", value: f }, S = new Set(n), I = {}, y = (A) => {
    ...
    h = !!b.readOnly, a.style.tabSize = b.tabSize || 2, s.inputMode = h ? "none" : "", s.setAttribute("aria-readonly", h), k(), T(), (c != (c = m[r]) || f != s.value) && (Z(), s.value = f, s.selectionEnd = 0, x());
  }, x = () => {
...
`;
    ...
    setSelection(A, R = A, L) {
      Z(), s.setSelectionRange(A, R, L), B(!0);
    },
    addExtensions(...A) {
      T(A);
    },
...
`, t)) + 1 ? t : e.length, Ha = (e, t, n) => (e.addListener(t, n), () => e.removeListener(t, n)), Va = (e, t) => parseFloat(getComputedStyle(e)[t]);
let he;
...
let Rt = !1;
const yn = navigator.clipboard, ce = Ke ? 4 : 2, Za = (e) => Rt = e, le = (e) => e.search(/\S|$/), qa = (e = ['""', "''", "``", "()", "[]", "{}"], t = /([^$\w'"`]["'`]|.[[({])[.,:;\])}>\s]|.[[({]`/s) => (n, r) => {
  let c;
  ...
  }, p = ([a, i], [l, o], s, b) => (a < i || !b && t.test((s[i - 1] || " ") + l + (s[i] || " "))) && !V(n, l + s.slice(a, i) + o, null, null, a + 1, i + 1), v = ([a, i], l, o) => a == i && o[i] == l && !n.setSelection(a + 1), h = (a, i, l, o, s, b) => {
    let S = i.join(`
`);
    if (S != a.join(`
`)) {
      const I = a.length - 1, y = i[I], x = a[I], T = x.length - y.length, k = i[0].length - a[0].length, O = l + le((k < 0 ? i : a)[0]), P = o - x.length + le(T > 0 ? y : x), C = l - o + S.length + T, Z = O > s ? s : Math.max(O, s + k), H = b + l - o + S.length;
      V(
        n,
        S,
        l,
  ...
  u["<"] = (a, i, l) => p(i, "<>", l, !0), e.forEach(([a, i]) => {
    ...
    o && (V(n, ">" + o, null, null, i[0] + 1), G(a));
  }, d.Tab = (a, [i, l], o) => {
    if (Rt || r.readOnly || Se(a) & 6) return;
    const [s, b] = w(r), S = a.shiftKey, [I, y, x] = te(o, i, l);
    return i < l || S ? _(S, I, y, x, i, l, s, b) : V(n, s.repeat(b - (i - y) % b)), g();
  }, d.Enter = (a, i, l) => {
    var s, b, S;
    const o = Se(a) & 7;
    if (!o || o == ce) {
      o && (i[0] = i[1] = te(l, i[1])[2]);
      const [I, y] = w(), [x, T] = i, k = (s = ct[dt(n)]) == null ? void 0 : s.autoIndent, O = Math.floor(le(It(l, x)) / y) * y, P = (b = k == null ? void 0 : k[0]) != null && b.call(k, i, l, n) ? y : 0, C = (S = k == null ? void 0 : k[1]) == null ? void 0 : S.call(k, i, l, n), Z = `
` + I.repeat(O + P) + (C ? `
` + I.repeat(O) : "");
      if (Z[1] || l[T])
        return V(n, Z, x, T, x + O + P + 1), g();
    }
  }, d.Backspace = (a, [i, l], o) => {
    if (i == l) {
      const s = It(o, i), b = r.tabSize || 2, S = e.includes(o.slice(i - 1, i + 1)), I = /[^ ]/.test(s) ? 0 : (s.length - 1) % b + 1;
      if (S || I > 1)
        return V(n, "", i - (S ? 1 : I), i + S), g();
    }
  };
  for (let a = 0; a < 2; a++)
...
` + x, I, y, l + T, o + T), g();
        ...
        return E.scrollBy(0, Va(E, "lineHeight") * (a ? 1 : -1)), !0;
    };
  z(n, "keydown", (a) => {
    var S;
    ...
    if (i == ce && (l == 221 || l == 219))
      ...
      const I = n.value, y = i == 9, x = y ? o : We(I, o), T = ct[dt(n, x)] || {}, { line: k, block: O } = ((S = T.getComments) == null ? void 0 : S.call(T, n, x, I)) || T.comments || {}, [P, C, Z] = te(I, o, s), H = P.length - 1;
      if (y) {
        if (O) {
        ...
        const B = Ie(k), D = RegExp(`^\\s*(${B} ?|$)`), X = RegExp(B + " ?"), A = !/\S/.test(I.slice(C, Z)), R = P.map(
...
`), Q = X + C, j = Q > o ? o : Math.max(o + L, Q), we = Q > s - (o != s) ? s : Math.min(Math.max(Q, s + L), C + q.length);
        V(n, q, C, Z, j, Math.max(j, we)), g(), G(a);
      }
    } else if (i == 8 + ce && l == 75) {
...
`, b, b, l + c.length + 1), g(), G(i)) : (yn.writeText(c = s), a == "cut" && (V(n, "", b, S + 1), g()), G(i));
      }
    })
  );
}, Ka = (e = 999) => {
