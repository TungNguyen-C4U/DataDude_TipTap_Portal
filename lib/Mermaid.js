...
import { v as Z, w as J, A as K, x as Y, y as Q, z as tt, B as et, F as rt, G as V, J as it, K as at } from "./RichTextEditor-D4neHvBX.js";
...
import { T as dt } from "./textarea-BBW4tXTD.js";
import { I as q, j as ht, k as W } from "./index-p_4uATkU.js";
const j = `graph TB
          ...
          /* @__PURE__ */ o(tt, { children: /* @__PURE__ */ o(
            et,
            {
              onClick: async () => {
                if (r !== "") {
                  if (r) {
                    const l = M.current.querySelector("svg"), { width: f, height: L } = l.getBoundingClientRect(), O = `mermaid-${lt()}.svg`;
...
function mt({ editor: e, node: t, updateAttributes: a, getPos: r, selected: g }) {
  const [_, N] = m({
    ...
    h: 0,
    dir: ""
  }), { align: k } = t == null ? void 0 : t.attrs, b = F(() => {
    const { src: i, alt: d, width: h, height: w } = t == null ? void 0 : t.attrs, v = $(h) ? `${h}px` : h, s = $(w) ? `${w}px` : w;
    return {
      src: i || void 0,
      alt: d || void 0,
      style: {
  ...
  const T = y(
    V((i) => {
      if (i.preventDefault(), i.stopPropagation(), !n)
        return;
      const { x: d, w: h, dir: w } = z, v = (i.clientX - d) * (/l/.test(w) ? -1 : 1), { width: s, height: u } = t == null ? void 0 : t.attrs, c = s / u, P = it(h + v, ht, _.width), U = Math.round(P / c);
      a({
        width: P,
        height: U
      });
  ...
  const D = F(() => new ResizeObserver(() => O()), [O]);
  return R(() => (D.observe(e.view.dom), () => {
    D.disconnect();
  }), [e.view.dom, D]), /* @__PURE__ */ o(
    st,
    {
      className: "image-view",
      style: { ...l, width: "100%", textAlign: k },
      children: /* @__PURE__ */ H(
        "div",
        {
          "data-drag-handle": !0,
